...
def run_job(req: SpreadsheetRequest):
    ...
    try:
        ...
        # ── Filter URL tidak valid SEBELUM loop utama ──
        valid_pool = []
        skipped = 0
        for idx, url in random.sample(url_index_pairs, min(req.batch_size or 50, total_antrean)):
            if extract_shortcode(url):
                valid_pool.append((idx, url))
            else:
                skipped += 1
        ...
        # ── Loop utama — SATU loop, tidak bersarang ──
        for idx, url in valid_pool:
            if not job_status["running"]:
                log("[JOB] Dihentikan paksa.")
                break

            # Jeda adaptif: lebih pendek jika streak sukses, lebih panjang jika baru gagal
            ...
            # Coffee break, skalanya menyesuaikan ukuran batch
